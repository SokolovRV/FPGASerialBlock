...
/*
	...
	Version change: fix interference stability.

	Max clk freq: 400 MHz
	Max data rate: 100 Mbit/s ( Max clk freq / 4 )
	Data packet length [bit]: 32 + (20 * n_words)
	
	Protocol:
		Data: Big-endian
		CRC16: polinom - 0x8005, initial - 0xFFFF
		Bytes in message follow without delay (bytes split one STOP bit and one START bit).
		If non-stop broadcast (START input always high level) - delay between messages is 11 bauds (bits);
...
localparam [15:0] polinom          = 16'h8005;
...
/*
	...
	Protocol:
		Data: Big-endian
		CRC16: polinom - 0x8005, initial - 0xFFFF
		Maximum delay between bytes in ONE message - 8 bauds(bits);
		Minimum delay between messages - 11 bauds(bits);
...
always @(posedge clk) begin
	...
	if(reset) errors_cnt <= 16'h0000;
	if(reset || tmout) begin
		state <= START;
		valid_lo <= 1'b0;
		validate <= 1'b0;
		data_strb <= 1'b0;
		...
		case(state)
			...
			DATA_LO: begin
				if(strb_bf && !pre_strb_0) begin
					...
					data_strb <= 1'b1;
					if(selector == max_wrd_indx && first_word) begin
						state <= CRC_HI;
					end
					else begin
						state <= DATA_HI;
			...
			CRC_LO: begin
				crc_reset <= 1'b1;
				data_strb <= 1'b0;
				if(crc_16[15:8] == crc_hi_bf[15:8]) begin
					valid_lo <= 1'b1;
				end	
				if(strb_bf && !pre_strb_0) begin
			...
			VALID: begin
				if((crc_16[7:0] == crc_hi_bf[7:0]) && valid_lo) begin
					validate <= 1'b1;
				end
				else begin
